<!-- filepath: /d:/laragon/www/appkasir/resources/views/penjualan/addtransaksi.blade.php -->
<x-header>Transaksi Penjualan</x-header>
<x-sidebar></x-sidebar>
...
<div class="container mt-5">
    ...
    <div class="card p-4">
        <!-- Form untuk transaksi penjualan -->
        <form action="{{ route('penjualan.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select id="select_produk" class="form-select">
                    ...
                    @foreach($produk as $item)
                        <option value="{{ $item->kode_produk }}" data-harga="{{ $item->harga_jual }}" data-stok="{{ $item->stok }}">
                            {{ $item->nama_produk }} ({{ $item->kode_produk }}) - Rp. {{ number_format($item->harga_jual, 0, ',', '.') }} (Stok: {{ $item->stok }})
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-3">
                <label for="select_member" class="form-label">Pilih Member:</label>
                <select id="select_member" class="form-select" onchange="updateMember()">
                    <option value="">-- Pilih Member --</option>
                    @foreach($members as $member)
                        <option value="{{ $member->id_member }}" data-diskon="10">
                            {{ $member->nama }} ({{ $member->kode_member }})
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="diskon" id="diskon" class="form-control" placeholder="Diskon" onchange="updateTotal()">
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="bayar" id="bayar" class="form-control" required oninput="updateKembalian()">
            </div>
...
<script>
    ...
    document.getElementById('tambah-produk').addEventListener('click', function() {
        ...
        if (existingProduk) {
            if (existingProduk.jumlah + 1 > produk.stok) {
                alert('Stok produk ' + produk.nama_produk + ' tidak mencukupi.');
                return;
            }
            existingProduk.jumlah += 1;
            existingProduk.subtotal = existingProduk.harga_jual * existingProduk.jumlah;
        } else {
            if (1 > produk.stok) {
                alert('Stok produk ' + produk.nama_produk + ' tidak mencukupi.');
                return;
    ...
    function updateTable() {
        ...
        selectedProduk.forEach((produk, index) => {
            ...
            tr.innerHTML = `
                ...
                <td>Rp. ${produk.harga_jual.toLocaleString()}</td>
                <td>
                ...
                <input type="hidden" name="produk[${index}][harga_jual]" value="${produk.harga_jual}">
            `;
    ...
    function updateJumlah(index, jumlah) {
        const produk = produkList.find(item => item.kode_produk == selectedProduk[index].kode_produk);
        if (jumlah > produk.stok) {
            alert('Stok produk ' + produk.nama_produk + ' tidak mencukupi.');
            return;
        }
        selectedProduk[index].jumlah = parseInt(jumlah);
        selectedProduk[index].subtotal = selectedProduk[index].harga_jual * selectedProduk[index].jumlah;
        updateTable();
        updateTotal();
    }
    ...
    function updateKembalian() {
        const totalHarga = parseInt(document.getElementById('total_harga').value.replace(/[^0-9]/g, '')) || 0;
    ...
    function updateMember() {
        const selectMember = document.getElementById('select_member');
        const selectedOption = selectMember.options[selectMember.selectedIndex];
        const diskon = selectedOption ? selectedOption.getAttribute('data-diskon') : 0;
        document.getElementById('diskon').value = diskon;
        document.getElementById('id_member').value = selectMember.value || null;
        updateTotal();
    }
</script>